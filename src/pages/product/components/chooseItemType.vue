<template>
  <div class="chooseItemType col-xs-12">
    <div class="cover" @click="handleItemTypeClick()"></div>
    <div class="container">
      <div class="top_title ">
        <div class="col-xs-12">
          <div class="col-xs-4 text-center">
            <img :src="this.defaultTypeShowImg" class="top_title_img">
          </div>
            ...
            <div v-for="(childItem, childIndex) in value">
              <div class="typeInfo">
                <div
                  ...
                  v-bind:class="{active:childIndex==isTypeActive[index].showItem}"
                  @click="chooseTheType(index,childIndex, childItem.id, childItem.type, childItem.value)">
                  <span v-if="childItem.img == ''">{{childItem.value }}</span>
                  <div v-else class="typeInfo_img" @click="imgPick(childItem.img)"><img v-lazy="childItem.img" alt=""></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="quantity text-center ">
        ...
        <div class="inputNumber iconfont text-right col-xs-4">
          ...
          <div class="inputNumber_input">
            <input type="text" :value="this.itemNum">
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      chooseTheType(index, childIndex, id, type, value) {
        console.log(index);
        if (index === 0) {
          this.isActive = childIndex;
          this.itemID = id;
          this.itemType = type;
          this.itemValue = value;
          this.chooseTypeOne = true;
          this.isTypeActive[index].showItem = childIndex;
          console.log(this.itemID, this.itemType, this.itemValue)
        }
        if (index === 1) {
          this.isActive2 = childIndex;
          this.itemID2 = id;
          this.itemType2 = type;
      ...
      imgPick(img){
        this.defaultTypeShowImg = img.replace("60x60","400x400");
        console.log(" this.topShowImg "+ this.defaultTypeShowImg )
      },

    },

    watch: {
      topShowImg: function (newVal, oldVal) {
        // console.log(newVal)
